...
print(get_confussion_matrix(bi_class_data['y'], bi_class_data['yp'], [1,2]))

# Multiclass
# The order's not right, but results are fine.
print(get_confussion_matrix(tri_class_data['y'], tri_class_data['yp'], TRICLASSES))

# Verification
confusion_matrix(tri_real, tri_pred)
...
## Verdaderos Positivos(VP), Verdaderos Negativos(VN), Falsos Positivos(FP) y Falsos Negativos(FN)
...
tri_matrix = get_confussion_matrix(tri_class_data['y'], tri_class_data['yp'], TRICLASSES)

tri_matrix = get_confussion_matrix(tri_class_data['y'], tri_class_data['yp'], TRICLASSES)
print(tri_matrix)
tri_counts = get_tri_counts(tri_matrix, [1,2,3])
print(tri_counts)
...
print("Accuracy: ", get_accuracy(bi_counts))

from sklearn.metrics import accuracy_score
accuracy_score(bi_real, bi_pred)
...
get_precision(bi_counts)

from sklearn.metrics import precision_score
# Average only necesary for multi-class
precision_score(bi_real, bi_pred)
...
target_names = ['1', '2']
print(classification_report(bi_real, bi_pred, target_names=target_names))
...
matthews_corrcoef(bi_real, bi_pred) # Pending verification.
...
print("Accuracy class 1: ", get_global_accuracy(tri_matrix))

from sklearn.metrics import accuracy_score
accuracy_score(tri_real, tri_pred)

def get_avg_accuracy(conf_matrix):
  n = sum(sum(conf_matrix.to_numpy()))
  c = len(conf_matrix.index)

  counts = get_counts(conf_matrix=conf_matrix)
  acc = 0
  for i in range(c):
    acc += counts[i]["VP"]
    acc += counts[i]["VN"]
...
print("Accuracy avg: ", get_avg_accuracy(tri_matrix))
...
print("Precision: ", get_multiclass_precision(tri_matrix, TRICLASSES))

from sklearn.metrics import precision_score
precision_score(tri_real, tri_pred, average='micro')
...
print("Especificidad: ", get_multiclass_specificity(tri_matrix, TRICLASSES))

from sklearn.metrics import classification_report
target_names = ['1', '2', '3']
report = classification_report(tri_real, tri_pred, target_names=target_names)
specificity = report.split("\n")[3].strip().split()[2]
specificity
...
print("Sensibilidad: ", get_multiclass_sensivity(tri_matrix, TRICLASSES))

from sklearn.metrics import classification_report
target_names = ['1', '2', '3']
report = classification_report(tri_real, tri_pred, target_names=target_names)
sensitivity = report.split("\n")[2].strip().split()[2] 
sensitivity
...
print("F1-Score: ", get_f1_multiclass(tri_matrix, TRICLASSES))

from sklearn.metrics import f1_score
f1_score(tri_real, tri_pred, average='micro')
...
def evaluate_multiclass(tindx, yr, yp, classes, pm):
  ...
  if(pm == 'f1'):
    ...
    return get_avg_accuracy(conf_matrix)
  elif(pm == "global_acc"):
